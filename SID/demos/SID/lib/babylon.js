...
class Scene {
    constructor(id_or_engine) {
        this.engineIsShared = (typeof id_or_engine !== 'string');
        if (this.engineIsShared) {
            this.engine = id_or_engine;
            this.canvas = this.engine.getRenderingCanvas();
        } else {
            this.canvas = document.getElementById(id_or_engine);
...
class IMUScene extends Scene {
    ...
    onSample(sample) {
        const q = new Quaternion(sample[this.options.signal]);
        this.imu.setQuat(q);
        if (this.options.axisSignal) {
            this.imu.setAxis(sample[this.options.axisSignal]);
        }
